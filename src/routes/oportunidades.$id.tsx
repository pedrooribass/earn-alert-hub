...
function DetailPage(){
  ...
  return <div className="app-frame">
    ...
    <main className="safe-bottom pb-32">
      ...
      <section className="px-5 pb-6"><h2 className="text-base font-bold">O essencial</h2><div className="mt-2 divide-y"><div className="detail-row"><Clock3/><span>Tempo estimado</span><strong>{item.time}</strong></div><div className="detail-row"><ShieldCheck/><span>Dificuldade</span><strong>{item.difficulty}</strong></div><div className="detail-row"><Bell/><span>Prazo</span><strong>{item.deadline}</strong></div></div></section>
      <section className="px-5 py-2"><h2 className="text-base font-bold">Como aproveitar</h2><ol className="mt-3 space-y-4">{item.steps.map((step,i)=><li key={step} className="flex gap-3"><span className="step-number">{i+1}</span><p className="pt-1 text-[13px] leading-5">{step}</p></li>)}</ol><div className="mt-6 flex gap-3 border-t pt-4"><Check className="mt-0.5 size-5 flex-none text-positive"/><p className="text-xs leading-5 text-muted-foreground">Oferta testada. Confirma sempre as condições finais no site da marca antes de começar.</p></div></section>
    </main>
    <div className="bottom-action"><Button asChild size="lg" className="h-12 w-full text-[15px]"><a href={item.url} target="_blank" rel="noreferrer" onClick={()=>current==="not-started"&&setProgress(item.id,"in-progress")}>{current==="in-progress"?`Continuar para ganhar ${item.rewardLabel}`:`Começar e ganhar ${item.rewardLabel}`}<ExternalLink/></a></Button></div>
  </div>;
}