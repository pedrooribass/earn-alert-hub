...
function TodayPage() {
  ...
  return <AppShell action={<Button asChild variant="ghost" size="icon" aria-label="Abrir alertas"><Link to="/alertas"><BellRing /></Link></Button>}>
    <section className="px-5 pb-6 pt-2 motion-rise">
    ...
    <section className="pb-6"><div className="section-heading section-heading-stacked"><div><p className="eyebrow">Ordenadas para ti</p><h2 className="mt-1.5 text-xl font-bold">As tuas melhores oportunidades</h2></div><Link to="/explorar" className="text-xs font-semibold text-primary">Ver todas</Link></div><div className="featured-grid px-5">{featured.map((item,index)=><Link key={item.id} to="/oportunidades/$id" params={{id:item.id}} className="featured-opportunity"><div className="flex items-center justify-between"><BrandLogo name={item.brand} large/><span className="featured-kicker">{index===0?"Melhor oportunidade":"Melhor retorno"}</span></div><h3 className="mt-4 text-base font-semibold">{item.brand}</h3><p className="mt-1 value-display text-primary-foreground">{item.rewardLabel}</p><p className="mt-1 text-xs text-primary-foreground/70">{item.rewardType}</p><div className="mt-4 flex items-center justify-between"><span className="flex items-center gap-1.5 text-xs opacity-75"><Clock3 className="size-3.5"/>{item.time}</span><span className="featured-action">Começar <ArrowRight/></span></div></Link>)}</div></section>
