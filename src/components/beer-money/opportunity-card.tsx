...
export function OpportunityCard({ item, compact = false }: { item: Opportunity; compact?: boolean }) {
  ...
  return <article className={`opportunity-card ${compact ? "min-w-[312px] snap-start" : ""}`}>
    <div className="flex items-start justify-between gap-3">
      <div className="flex min-w-0 items-center gap-4"><BrandLogo name={item.brand} large /><div className="min-w-0"><p className="truncate text-lg font-bold">{item.brand}</p><p className={`mt-1 text-[11px] font-semibold ${personalStatus === "completed" ? "text-muted-foreground" : "text-positive"}`}>{progressLabels[personalStatus]}</p></div></div>
    ...
    <p className="mt-2 line-clamp-2 text-sm leading-5 text-muted-foreground">{item.summary}</p>{item.insight && <p className="mt-2 text-xs font-semibold text-primary">{item.insight}</p>}
    ...
    <Button asChild variant={personalStatus === "completed" ? "secondary" : "default"} className="mt-5 h-11 w-full justify-between"><Link to="/oportunidades/$id" params={{ id: item.id }}>{personalStatus === "not-started" ? "Começar" : personalStatus === "in-progress" ? "Continuar" : "Ver conclusão"}<ArrowRight /></Link></Button>
  </article>;
}
