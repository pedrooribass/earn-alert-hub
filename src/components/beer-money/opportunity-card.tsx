...
export function OpportunityCard({ item, compact = false }: { item: Opportunity; compact?: boolean }) {
  ...
  return <article className={`opportunity-card ${compact ? "min-w-[312px] snap-start" : ""}`}>
    ...
    <p className="mt-1.5 line-clamp-2 text-[13px] leading-[19px] text-muted-foreground">{item.summary}</p>{item.insight && <p className="mt-1.5 text-xs font-semibold text-primary">{item.insight}</p>}
    <p className="mt-4 value-display">{item.rewardLabel}</p><p className="mt-1 text-[11px] font-medium text-muted-foreground">{item.rewardType}</p>
